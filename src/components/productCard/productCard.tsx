...
export const ProductCardPage: React.FC = () => {
  ...
  useEffect(() => {
    axiosInstance.get('/oils/' + params.id).then((res) => {
        ...
        <div className="flex flex-col items-center md:items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2 text-gray-300">
              <p>
                <strong className="font-medium text-white">Viscosity Grade:</strong>{' '}
                {productCard?.specifications.viscosityGrade}
              </p>
              <p>
                <strong className="font-medium text-white">Additive Source:</strong>{' '}
                {productCard?.specifications.additiveSource}
              </p>
              <p>
                <strong className="font-medium text-white">Industry Standards:</strong>{' '}
                {productCard?.specifications.industryStandards.join(', ')}
              </p>
            </div>

            <h2 className="text-2xl font-semibold mt-8 mb-3">Packaging</h2>
            <h3 className="text-gray-300">{productCard?.packaging}</h3>
